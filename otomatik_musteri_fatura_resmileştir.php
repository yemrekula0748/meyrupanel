...
while ($islemSayisi < $maxKayit) {
    ...
    $siparis = $db->fetchAssoc(
        $db->query("SELECT * FROM siparisler WHERE parasut_resmilesme_durumu = 0 AND iptalmi = 0 AND hangikargo = 'MeyruKids' AND (parasut_fatura_numarasi IS NULL OR parasut_fatura_numarasi = '') AND (sales_invoice_id IS NULL OR sales_invoice_id = '') AND resmimi = 0 AND resmilestir = 0 AND kargo = 'Ödeme Şartlı' LIMIT 1")
    );
    if (!$siparis) {
