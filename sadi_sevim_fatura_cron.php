...
$siparisQuery = $db->query("SELECT * FROM siparisler WHERE hangikargo = 'Sevim Aydın - PTT' AND parasut_id != 0 AND parasut_fatura_numarasi IS NULL AND kargo = 'Ödeme Şartlı' LIMIT 1");
$siparisler = [];
...
foreach ($siparisler as $siparis) {
    ...
    if (curl_errno($ch)) {
        ...
        $updateQuery = $db->query("UPDATE siparisler SET parasut_fatura_numarasi = '" . $response_data['data']['attributes']['invoice_no'] . "' WHERE id = '" . $siparis['id'] . "'");
    }
